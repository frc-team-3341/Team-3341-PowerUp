...
bool MoveLiftToMinHeight::IsFinished() {
	...
	//{
		//cout << "reached IsFinished" << endl;
		//return true;
	//}
	//else

	/*
	ControlMode test = lift->getMotor()->GetControlMode();
	if(test == ControlMode::Disabled)
	{
		cout << "IsFinished is true--------------------------------------" << endl;
		return true;
	}
	else
		return false;
	...
	/*
	currentPos = lift->getPosition();
	cout << "currentPos = " << currentPos << ",     tempPos = " << tempPos << endl;
